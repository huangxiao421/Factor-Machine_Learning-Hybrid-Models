...
import warnings
# ignores ICA, no convergence warning
...
def main(data, tune, variation): 
    ...
    if tune == True:
        ...
        for column in data["house_price"]:
            ...
            all_combinations, best_combination = elastic_net_tuning(input_data, target_data, variation, START_T)
    ...
    for t in range(START_T, len(input_data)):
        ...
        for column in data["house_price"]:
            ...
            if tune == True:
                model = ElasticNet(alpha = hyperparameters[column]["alpha"], l1_ratio = hyperparameters[column]["l1_ratio"], max_iter = 10_000)
            else:
                model = ElasticNet()
            ...
            parameters_t.append(hyperparameters[column]["alpha"])
            parameters_t.append(hyperparameters[column]["l1_ratio"])
...
def main_tune_every_t(data, tune, variation): 
    ...
    for t in range(START_T, len(data["key_figures"])):
        ...
        for column in data["house_price"]:
            ...
            if tune == True:
                ...
                input_data = data["key_figures"]
                target_data = data["house_price"][column]
                all_combinations, best_combination = elastic_net_tuning(input_data, target_data, variation, t)
    ...
    parameters = pd.DataFrame(parameters, columns = ["Date", "price_index_ALPHA", "price_index_L1_RATIO", "price_index_K", "", "avg_selling_price_ALPHA", "avg_selling_price_L1_RATIO", "avg_selling_price_K"])
    parameters.drop(parameters.columns[4], axis = 1, inplace = True)
...
def elastic_net_tuning(input_data, target_data, variation, t):
    # define model (alpha = lambda = weight penalties; l1_ratio = alpha = ratio l1/l2 penalties)
    alpha_list = [0.1, 0.25, 0.5, 1, 1.5]
    l1_ratio_list = [0.1, 0.25, 0.5, 0.75, 0.9]
    # Get all combinations
    hyperparameters = list(itertools.product(*[alpha_list, l1_ratio_list]))
    ...
    best_combination = {"alpha" : best_combination[0], "l1_ratio" : best_combination[1]}
    
    # Create table with corresponding tune results
    tune_results = np.asarray(mse_list).reshape(len(alpha_list), len(l1_ratio_list))
    tune_results = pd.DataFrame(tune_results, index = alpha_list, columns = l1_ratio_list )    
...
def make_joint_plot(joint_results, tet):
    ...
    for target in ["price_index", "avg_selling_price"]:
        ...
        # Specifications
        plt.xticks(rotation = 45)    
        plt.title(f"{target.upper()} - Elastic Net {tet}")
        plt.ylabel('Growth rate')
        plt.legend() 
        plt.show()
...
make_individual_plots(pca_results, "Elastic Net", "PCA")
make_individual_plots(spca_results, "Elastic Net", "SPCA")
make_individual_plots(ica_results, "Elastic Net", "ICA")

make_individual_plots(pca_results_TET, "Elastic Net", "PCA")
make_individual_plots(spca_results_TET, "Elastic Net", "SPCA")
make_individual_plots(ica_results_TET, "Elastic Net", "ICA")
...
if SAVE == True:
    save_dictionary_to_excel(all_results, file_name = "Hybrid_Elastic_Net_2")
